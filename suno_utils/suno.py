...
def generate_song(
    ...
    """
    Generate a song on Suno HackMIT API and poll until streaming or complete.

    Returns the clip object (dict) from the /clips response when status is "streaming" or "complete".

    Raises SunoAPIError on non-recoverable failures or on timeout.
    """
    if api_key is None:
        api_key = os.environ.get("SUNO_API_KEY")
    if not api_key:
        raise SunoAPIError("Suno API key not provided. Set SUNO_API_KEY env var or pass api_key.")
...
if __name__ == "__main__":
    ...
    stream_audio(clip_result.audio_url)